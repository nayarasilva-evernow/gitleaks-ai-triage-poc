...
def print_gate_result(ok: bool, tps: int, uncertain: int) -> None:
    ...
    if ok:
        ...
        print(f"     {carta}")
        print("  5. Veja suggested_rules.toml e o Summary desta run")
    print("=" * width)
    print()
    ...
    if not ok:
        ...
        lines.extend(
            [
                ...
                "",
                "Siga o guia de atendimento (categoria AppSec / Secret Scanning):",
                "",
                f"→ **[Como abrir chamado — Cybersegurança]({chamado})**",
                "",
                "No chamado, inclua: link do PR, URL desta run, veredicto da triagem,",
                "arquivo/regra/motivo e o que já foi feito (remoção, rotação, allowlist).",
                "",
                "### Se Cyber solicitar carta de risco",
                "",
                "Use o modelo preenchível abaixo, assine com a área dona do risco",
                "e anexe ao mesmo chamado:",
                "",
                f"→ **[Carta de Aceite de Risco]({carta})**",
                "",
            ]
        )
